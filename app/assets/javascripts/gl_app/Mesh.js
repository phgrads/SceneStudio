...
Mesh.GenerateCube = function(gl, xform) {
  //    v6----- v5
  //   /|      /|
  //  v1------v0|
  //  | |     | |
  ...
  var attribArray = new Float32Array(nVerts * 8);
  for (var i = 0; i < nVerts; i++) {
    var aBase = 8*i;
    var vBase = 3*i;
    var tBase = 2*i;

    attribArray[aBase+0] = vertices[vBase+0];
    attribArray[aBase+1] = vertices[vBase+1];
    attribArray[aBase+2] = vertices[vBase+2];
    attribArray[aBase+3] = texCoords[tBase+0];
    attribArray[aBase+4] = texCoords[tBase+1];
    attribArray[aBase+5] = normals[vBase+0];
    attribArray[aBase+6] = normals[vBase+1];
    attribArray[aBase+7] = normals[vBase+2];
  }

  if (xform) {
    Mesh.TransformAttribArray(attribArray, xform, Mesh.DEFAULT_VERTEX_FORMAT);
  }

  return new Mesh(gl, attribArray, indices, Mesh.DEFAULT_VERTEX_FORMAT);
};
