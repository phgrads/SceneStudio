class Experiments::SelectSceneController < ApplicationController
  ...
  include Experiments::RateSceneHelper
  ...
  def index
    ...
    @title = @task.title
    if @entries.any? then
      render "experiments/rate_scene/index", layout: true
    else
  ...
  def results
    render "experiments/rate_scene/results", layout: true
  end

  def view
    render "experiments/rate_scene/view", layout: true
  end
